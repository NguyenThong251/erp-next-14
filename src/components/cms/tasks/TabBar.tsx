...
const tabs: TabItem[] = [
  ...
  { id: "assigned", label: "Việc giao cho tôi", count: 4 },
  { id: "pending", label: "Việc chờ duyệt" },
  { id: "delegated", label: "Việc tôi giao", count: 4 },
  ...
  return (
    <>
      <div className="bg-[#fff] px-5 rounded-[16px]  flex items-center justify-between ">
        <div className="flex gap-[50px]">
          {tabs.map((tab) => (
            ...
            >
              ...
              {activeTab === tab.id && (
                <div className="absolute -bottom-[7px] w-[100%] h-1 rounded-t-[5px] bg-[#2275FF]"></div>
              )}
            </div>
          ))}
        </div>
        {/* Nút Thêm công việc */}
        <div className=" py-5 ">
          <button className=" flex items-center  bg-gradient-to-r from-[#21D4FD] to-[#2152FF] text-white text-sm font-medium rounded-[10px] px-4 py-2 ">
            <svg
